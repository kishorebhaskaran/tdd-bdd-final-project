...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        ...
        self.assertEqual(productdata["name"], test_product.name)

    def test_update_product(self):
        """Test for updating an existing product"""
        # creating a product to update
        test_product = ProductFactory()
        response = self.client.post(BASE_URL, json=test_product.serialize())
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        # updating the product
        new_product = response.get_json()
        new_product["description"] = "brand new"
        response = self.client.put(f"{BASE_URL}/{new_product['id']}", json=new_product)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        updated_product = response.get_json()
        self.assertEqual(updated_product["description"],"brand new")
    ...
    def test_query_by_category(self):
        """Teest Query Products by category"""
    ...
    def test_query_by_availability(self):
        """Test Query Products by availability"""
        products = self._create_products(10)
